...
import sys
from dbustil import get_conn, close_conn
import hashlib
import re
import datetime
...
class MainApp(QMainWindow, ui):
    ...
    def editor_user(self):
        ...
        if user_pwd == user_confirm_pwd:
            ...
            if user_pwd == user_confirm_pwd:
                ...
                # 4、执行语句
                cur.execute(sql, (new_name, new_email,
                                  user_pwd_hash, old_name))
                # 5、insert、update、delete必须显示提交
                conn.commit()
                # 6、关闭资源
                close_conn(conn, cur)
    ...
    def editor_client(self):
        ...
        if re.match(regex, client_phone):
            ...
            # 4、执行语句
            cur.execute(sql, (client_name, client_phone,
    ...
    def handel_day_operation(self):
        ...
        sql = "insert into dayoperations(book_name, client_name, type, days, day_from, day_to)" \
              "values(%s, %s, %s, %s, %s, %s)"
        cur.execute(sql, (book_name, client_name, operate_type,
                          day_number, today_date, to_day))
        conn.commit()
        close_conn(conn, cur)
